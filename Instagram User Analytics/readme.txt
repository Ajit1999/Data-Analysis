This project is about the data analysis using SQL. The dataset is about data related to instagram users, photos posted by their user, likes on it, comment on it, hash tags etc. And they want some insights for marketing, product & development teams. 
These insights are then used by teams across the business to launch a new marketing campaign, decide on features to build for an app, track the success of the app by measuring user engagement and improve the experience altogether while helping the business grow.
This analysis will help them make a decision based on different metrics and insights. 

Tech-Stack Used
1. MySQL 8.0 command line
2. MySQL Workbench 8.0

APPROACH
For this project, I have used My SQL to extract the required data from the given database using the Join function, subqueries, Aggregation, where condition, Group by, Distinct and other functions required. keeping the Primary key and foreign key in consideration provided all the reports asked by the marketing department and Investor metrics department.

Insights and Results
From this project, I got an idea about how as a business or data analyst we work on real-time data to take any data-driven decision. One thing I infer about this project is the dataset provided was very limited and small in respect of Rows and columns, But still, it was a very good experience working on such kind of project.
It helped me a lot to understand the analysis process well, and to provide insights for the best decision possible